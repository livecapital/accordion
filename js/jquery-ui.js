...
/* PLUGINS! NÃO APAGAR DAQUI PRA BAIXO!!!*/
;    (function( $ ) {
        $.widget( "ui.combobox", {
            opts: {
                button: false,
            ...
            _create: function() {
                $.extend(this.opts, this.options);
                var helpText = this.options.helpText
                var counter = 0;
                var currentValue = 0;
                var currentText = "";
                if (this.options.defaultValue == ""){
                    ...
                    var self = this,
                        ...
                        value = selected.val() ? selected.text() : helpText;
                }
                var input = this.input = $( "<input>" )
                    ...
                    .focus(function(){
                        if(this.value == helpText){
                            this.value = "";
                            $(this).removeClass('ui-autocomplete-input-placeholder');
                        }
                    }).blur(function(){
                        if(this.value == ""){
                            actualSelected = select.children( ":selected" ),
                            this.value = (helpText)? helpText: actualSelected.html();

                            if(helpText)
                                $(this).addClass('ui-autocomplete-input-placeholder');
                        }
                    })
                    .autocomplete({
                        delay: 0,
                        ...
                        change: function( event, ui ) {
                            if ( !ui.item ) {
                                ...
                                if ( !valid ) {
                                    ...
                                    //$( this ).val( helpText );
                                    select.val( 0 );
                                    //input.data( "autocomplete" ).term = helpText;
                                    return false;
                                }
                            }
                        }
                    ...
                    if(input.val() == helpText){
                        input.addClass('ui-autocomplete-input-placeholder');
                    }
                ...
                if(this.options.button){
                    this.button = $( "<a href='javascript:void(0);'>&nbsp;</a>" )
                        .attr( "tabIndex", -1 )
